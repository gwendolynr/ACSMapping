...
#   mutate(City = str_replace(City, "JODAN", "JORDAN")) %>% 
#   mutate(City = str_replace(City, "SOUTH SALT LAKE CITY", "SOUTH SALT LAKE")) %>% 
#   mutate(City = toupper(City))  
  
# test <- subsidy %>% 
#   group_by(City) %>% 
#   summarize(total = sum(Subsidized.Amt, na.rm=TRUE))
# 
#  test <- test %>% 
#    filter(total!= 0) %>% 
#    mutate(state = "UTAH") %>% 
#    unite(address, City, state, sep= ", ")
#  
#  write.csv(test, "./raw_data/cities.csv")
...
spdf <- SpatialPointsDataFrame(coords = xy, data = subsidypoints,
 ...
 new_df <- point.in.poly(spdf, schooldistrictunified) 
...
```{r bachelor's degree, echo=FALSE}
